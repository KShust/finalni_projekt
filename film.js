const filmy = [
	...
	{
		id: 'duna-2',
		nazev: 'Duna: Část druhá',
		plakat: {
			url: 'https://image.pmgstatic.com/cache/resized/w420/files/images/film/posters/168/608/168608387_g1c1zx.jpg',
			sirka: 420,
			vyska: 592,
		},
		ochutnavka: 'Epická sága pokračuje!',
		popis:
...
poznamka.addEventListener('submit', overeni)
//----------8----------//
//---------6-----------//
...
document.getElementById('premiera').textContent = `Premiera bude za ${doPremiery} ${sklonovani(doPremiery)}`

if (doPremiery < 0) {
    document.getElementById('premiera').textContent = `Premiéra byla před ${Math.abs(doPremiery)} ${sklonovani(doPremiery)}`
} 
//---------6---------//
//---------7---------//
...
hvezdicky.forEach((hvezda, index) => {
    hvezda.addEventListener('click', () => {
    	posledni = index + 1
    	hvezdickyFce(posledni)
    })

	hvezda.addEventListener('mouseenter', () => {
		hvezdickyFce(index + 1)
	});
...
play.addEventListener('click', () => {
	if (document.body.contains(prehravac)) {
    	video.play();
...
video.addEventListener('timeupdate', () => {
    ...
    const cas = Math.floor(video.currentTime)
    let mins, secs
		mins = Math.floor(cas / 60)
    	secs = cas % 60
    		if (secs < 10) {
    		secs = '0' + String(secs)
  			}
...
document.addEventListener('mousemove', resetCasovac)
document.addEventListener('keydown', resetCasovac)